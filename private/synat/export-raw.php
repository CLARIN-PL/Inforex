...
$opt = new Cliopt();
$opt->addExecute("php export-raw.php --corpus n --user u --db-name xxx --db-user xxx --db-pass xxx --db-host xxx --db-port xxx",null);
...
foreach ($reports as $r){
  $filename = $config->folder . "/" . str_pad($r['id'],8,"0", STR_PAD_LEFT) . ".xml";
  $content = $r['content'];
  $content = str_replace("<sentence>", "", $content);
  $content = str_replace("</sentence>", "", $content);
