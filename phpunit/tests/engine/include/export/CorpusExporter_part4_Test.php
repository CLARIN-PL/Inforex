...
class CorpusExporter_part4_Test extends PHPUnit_Framework_TestCase
{
...
// function export_document($report_id, &$extractors, $disamb_only, &$extractor_stats, &$lists, $output_folder, $subcorpora, $tagging_method){
    private function export_document($disamb_only,$tagging_method)
    {
        ...
        $extractorData = array(
            array( "report_id"=>$report_id, "id"=>1, "relation_type_id"=>1, "source_id"=>1, "target_id"=>100, "relation_set_id"=>1, "name"=>"nazwa relacji 1", "rsname"=>"relation set name 1"),
            array( "report_id"=>$report_id, "id"=>2, "relation_type_id"=>1, "source_id"=>100, "target_id"=>1, "relation_set_id"=>1, "name"=>"nazwa relacji 2", "rsname"=>"relation set name 2"),
        );
        $extractorObj->setExtractorReturnedData('relations',$extractorData);
    ...
    private function checkFiles($report_id,$disambOnly,$tagging_method,$reportData,$extractorData) {
        ...
        //checkJSONFile
        $expectedContent = $scl->toJSON(null,$extractorData);
...
} // CorpusExporter_part4_Test class
