                            ...
                            <div class="panel-body" style="">
                                ...
                                <select name="corpus_flag_id" style="font-size: 12px">
                                    ...
                                    {foreach from=$corpus_flags item=flag}
                                        <option value="{$flag.short}"
                                                {if $flag.short==$corpus_flag_id}selected="selected"{/if}
                            ...
                            <div class="panel-body" style="">
                                ...
                                {if $annotators|@count == 0}
                                    {capture assign=message}
                                        <em>There are no users with agreement attributes for the selected criteria.</em>
                                    {/capture}
                                    {include file="common_message.tpl"}
                                {else}
                                    {if $globalPSC}
                                        <h5>Annotators PSA in selected documents:
                                            <b>{$globalPSC|string_format:"%.2f"}</b></h5>
                                    {/if}
                                    <table class="tablesorter" cellspacing="1" style="width: 100%; margin-top: 6px;">
                                        ...
                                        {foreach from=$annotators item=a}
                                            <tr{if $a.user_id == $annotator_a_id} class="user_a"{elseif $a.user_id == $annotator_b_id} class="user_b"{/if}>
                                                ...
                                                <td style="line-height: 20px; text-align: center">{$a.annotation_count}</td>
                                                <td style="text-align: center;"><input type="radio" name="annotator_a_id" value="{$a.user_id}"
                                                                                       {if $a.user_id == $annotator_a_id}checked="checked"{/if}/>
                                                </td>
                                                <td style="text-align: center;"><input type="radio" name="annotator_b_id" value="{$a.user_id}"
