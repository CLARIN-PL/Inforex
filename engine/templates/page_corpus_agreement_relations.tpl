	                    ...
	                    {if $annotator_a_id && $annotator_b_id}
							...
							<table id="agreement" class="tablesorter" cellspacing="1">
                                ...
                                {foreach from=$agreement key=report_id item=relations}
                                    <tr>
                                        <th colspan="15" style="text-align: center; background-color: #FFB347">Report {$report_id}</th>
                                    </tr>
                                    {foreach from=$relations key=span item=relation}
                                        <tr>
                                            <th colspan="15" style="text-align: center; background-color: #D9EDF7">
                                                <div style = "display: inline-block;">[{$relation.data.source_bounds}]</div>
                                                ...
                                                <div style = "display: inline-block;">[{$relation.data.target_bounds}]</div>
                                            </th>
                                        </tr>
                                        <tr>
                                            <td colspan = "5" class = "text-center {if !empty($relation.a)}user_a{/if}" style = "border-right: 1px solid #bce8f1;">
                                            {foreach from=$relation.a key=relation_type_id item=rel}
                                                {$rel.name}<br>
                                            {/foreach}
                                            </td>
                                            <td colspan = "5" class = "text-center {if !empty($relation.a_and_b)}a_and_b{/if}" style = "border-right: 1px solid #bce8f1;">
							...
							<div class="panel panel-default" style="margin: 5px;">
								...
								<div class="panel-body" style="">
									...
									<div style="vertical-align: middle; line-height: 20px">
										...
										<label><input type="checkbox" class = "subcorpus_id" name="subcorpus_ids[]" value="{$subcorpus.subcorpus_id}" {if in_array($subcorpus.subcorpus_id, $subcorpus_ids)}checked="checked"{/if} /> {$subcorpus.name}</label>
										{/foreach}
									</div>
								</div>
							</div>
