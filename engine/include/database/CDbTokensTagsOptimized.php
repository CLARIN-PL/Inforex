...
class DbTokensTagsOptimized
{
    ...
    static function prepareReportSummary($user_a, $user_b, $report_id)
    {
        ...
        if (isset($content['format']) && $content['format'] == 'plain') {
            $content = htmlspecialchars($content);
        }
        $html = new HtmlStr2($content);

        self::prepareDataForUserAorB($user_a, $all, $html, 'a');
        self::prepareDataForUserAorB($user_b, $all, $html, 'b');
     ...
     * @param array $all
     * @param HtmlStr2 $html
     * @param string $userTag 'a' or 'b'
     */
    public static function prepareDataForUserAorB($user, array &$all, HtmlStr2 $html, $userTag='')
    {
        foreach ($user as $u) {
            $all[$u['token_id']] [$userTag] [] = $u;
            $all[$u['token_id']]['from'] = $u['from'];
            $all[$u['token_id']]['to'] = $u['to'];
            $all[$u['token_id']]['report_id'] = $u['report_id'];

            $all[$u['token_id']]['orth'] = $html->getTextAlign($u['from'], $u['to'], false, false);
        }
    }
